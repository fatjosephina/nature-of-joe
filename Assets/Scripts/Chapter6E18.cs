...
public class Chapter6E18 : MonoBehaviour
{
    public float maxSpeed = 2f, maxForce = 2f;

    public Mesh coneMesh; // If you want to use your own cone mesh, drop it into the editor here.

    private List<BoidA> boids; // Declare a List of Vehicle objects.
    ...
    {
        findWindowLimits();
        boids = new List<BoidA>(); // Initilize and fill the List with a bunch of Vehicles
        for (int i = 0; i < 100; i++)
        {
            float ranX = Random.Range(-1.0f, 1.0f);
            float ranY = Random.Range(-1.0f, 1.0f);
            boids.Add(new BoidA(new Vector2(ranX, ranY), minimumPos, maximumPos, maxSpeed, maxForce, coneMesh));
        }
    }
    ...
    void Update()
    {
        ...
        {
            boids.Add(new BoidA(mousePos, minimumPos, maximumPos, maxSpeed, maxForce, coneMesh));
        }
    }
...
class BoidA
{
    ...
    public BoidA(Vector2 initPos, Vector2 _minPos, Vector2 _maxPos, float _maxSpeed, float _maxForce, Mesh coneMesh)
    {
    ...
    public void Flock(List<BoidA> boids)
    {
        maxSpeed = Mathf.PerlinNoise(Random.Range(0f, 20f), Random.Range(0f, 20f)) * 10f;
        maxForce = Mathf.PerlinNoise(Random.Range(0f, 20f), Random.Range(0f, 20f)) * 10f;

        Vector2 sep = Separate(boids); // The three flocking rules
        Vector2 ali = Align(boids);
        Vector2 coh = Cohesion(boids);

        sep *= Mathf.PerlinNoise(Random.Range(-20f, 20f), Random.Range(-20f, 20f)) * 10f;
        ali *= Mathf.PerlinNoise(Random.Range(-5f, 5f), Random.Range(-5f, 5f)) * 5f;
        coh *= Mathf.PerlinNoise(Random.Range(-2f, 2f), Random.Range(-2f, 2f)) * 2f;

        Debug.Log("Max Speed: " + maxSpeed + ", Max Force: " + maxForce + ", Sep Weight: " + sep + ", Ali Weight: " + ali + ", Coh Weight: " + coh);
    ...
    public Vector2 Align(List<BoidA> boids)
    {
    ...
    public Vector2 Cohesion(List<BoidA> boids)
    {
    ...
    public Vector2 Separate(List<BoidA> boids)
    {
        Vector2 sum = Vector2.zero;
        int count = 0;
